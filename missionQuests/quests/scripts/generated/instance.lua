...
function onInit()
  ...
  self.warpEntity = quest.parameters().questGiver.uniqueId
  ...
  self.turnInEntity = quest.parameters().questGiver.uniqueId
...
function enterInstance(dt)
  ...
  -- })
  objective("enterInstance"):complete()
  ...
  self.onInteract = function(entityId)
    if world.entityUniqueId(entityId) == self.warpEntity then
      if not self.warpConfirmation then
        local dialogConfig = self.warpDialog
		dialogConfig.labels.name = quest.parameters().questGiver.name
		dialogConfig.images.portrait = quest.parameters().questGiver.portrait
